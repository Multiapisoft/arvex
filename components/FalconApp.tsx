    ...
    >
      ...
      <section>
        ...
        <div className="card" style={{ marginBottom: "1rem" }}>
          ...
          {userLoading || busy ? (
            <DataLoading label="Loading balance…" />
          ) : (
            <>
              <div className="stat-label !text-left">Available to withdraw</div>
              <div
                className="stat-value !text-left text-gradient-gold"
                style={{ fontSize: "2rem", marginBottom: "0.35rem" }}
              >
        ...
        <div className="card">
          ...
          <div className="flex flex-wrap gap-2">
            {account ? (
              <a
                className="btn btn-primary"
                href={`${explorerAddress(account, EXPLORER_BASE)}#tokentxns`}
                target="_blank"
                rel="noopener noreferrer"
              >
                My wallet · {shortAddr(account, 4)}
              </a>
            ) : (
            ...
            {isAddress(contractAddr) && contractAddr !== ZeroAddress ? (
              <a
                className="btn btn-ghost"
                href={explorerAddress(contractAddr, EXPLORER_BASE)}
                target="_blank"
                rel="noopener noreferrer"
              >
                Contract · {shortAddr(contractAddr, 4)}
              </a>
            ) : null}
          </div>
        </div>
        ...
        <div className="grid-two" style={{ marginBottom: "1rem" }}>
          ...
          <div className="card">
            ...
            <div className="grid-two" style={{ marginTop: "0.85rem" }}>
              ...
              <div>
                <div className="stat-label !text-left">Payable Amount</div>
                <div className="stat-value !text-left" style={{ fontSize: "1.15rem" }}>
                  {fmtUsd(sfPayableDisplay, tokenDecimals)}
                </div>
              </div>
              <div>
                <div className="stat-label !text-left">Target Amount</div>
                <div className="stat-value !text-left" style={{ fontSize: "1.15rem" }}>
                  {fmtUsd(sfTargetDisplay, tokenDecimals)}
                </div>
              </div>
              <div>
